...
if [ "$HOST_UID" ]; then
    usermod -u $HOST_UID travis
fi
if [ "$HOST_GID" ]; then
    groupmod -g $HOST_GID travis
fi
# This makes sure that all files in /home/travis are accessible by the user
...
find /home/travis -maxdepth 1 | sed "1d" | grep -v "/home/travis/shared" | xargs chown -R travis:travis 2> /dev/null || true
